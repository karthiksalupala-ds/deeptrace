...
import { Activity, FileUp, FolderOpen, History, LayoutDashboard, Settings, ShieldCheck, Store } from 'lucide-react'
import { getJob, getJobs, getVendors, Job, Vendor } from './lib/api'
...
const nav = [{ id: 'dashboard', label: 'Dashboard', icon: LayoutDashboard }, { id: 'upload', label: 'Upload evidence', icon: FileUp }, { id: 'history', label: 'Case history', icon: History }, { id: 'vendors', label: 'Vendors', icon: Store }]
type View = 'dashboard' | 'upload' | 'history' | 'vendors' | 'job' | 'report'
...
export function App() {
  ...
  useEffect(() => { refreshJobs(); void getVendors().then((result) => setVendors(result.vendors)).catch(() => setVendors([])) }, [])
  ...
  return <div className="app-shell"><aside className="sidebar"><div className="brand"><div className="brand-icon"><ShieldCheck size={21} /></div><div><strong>DEEPTRACE</strong><small>FORENSIC RECOVERY</small></div></div><div className="side-label">WORKSPACE</div><nav>{nav.map(({ id, label, icon: Icon }) => <button key={id} className={view === id || (id === 'dashboard' && view === 'job') ? 'active' : ''} onClick={() => setView(id as View)}><Icon size={17} />{label}</button>)}</nav><div className="sidebar-bottom"><div className="system-status"><span className="online-dot" /><div><strong>Engine online</strong><small>Local API connected</small></div></div><button className="settings-button"><Settings size={16} /> Settings</button></div></aside><main className="main-content"><header className="topbar"><div className="breadcrumb"><span>Evidence workspace</span><b>/</b><strong>{view === 'upload' ? 'New analysis' : view === 'history' ? 'Case history' : view === 'vendors' ? 'Vendor Matrix' : currentJob?.filename || 'Case review'}</strong></div><div className="topbar-meta"><Activity size={15} /> Read-only mode <span className="avatar">OP</span></div></header>{view === 'upload' && <Upload onJobCreated={onJobCreated} />}{view === 'history' && <HistoryPage jobs={jobs} onOpen={openJob} />}{view === 'vendors' && <Vendors vendors={vendors} />}{view === 'job' && jobId && <JobView jobId={jobId} onReport={(job) => { setCurrentJob(job); setView('report') }} />}{view === 'report' && currentJob?.report && <Report jobId={currentJob.job_id} report={currentJob.report} />}{view === 'dashboard' && <Dashboard jobs={jobs} onOpen={openJob} onUpload={() => setView('upload')} />}</main></div>
}
