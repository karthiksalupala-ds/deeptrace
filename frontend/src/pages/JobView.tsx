...
import { fileUrl, getJob, Job, RecoveredFile, Report, searchEvidence } from '../lib/api'
...
export function JobView({ jobId, onReport }: JobViewProps) {
  ...
  return <div className="page job-page">
    ...
    {job.status !== 'done' ? <Processing stages={stages} active={stageIndex} /> : <DoneContent report={report} files={files} view={view} setView={setView} filter={filter} setFilter={setFilter} searchQuery={searchQuery} setSearchQuery={setSearchQuery} copied={copied} setCopied={setCopied} />}
  </div>
}
...
function DoneContent({ report, files, view, setView, filter, setFilter, searchQuery, setSearchQuery, copied, setCopied }: { report: Report; files: Report['recovered_files']; view: 'table' | 'grid'; setView: (view: 'table' | 'grid') => void; filter: string; setFilter: (value: string) => void; searchQuery: string; setSearchQuery: (value: string) => void; copied: string; setCopied: (value: string) => void }) {
  ...
  return <>
    <div className="stat-grid"><Stat icon={<ShieldCheck />} label="Recovery rate" value={`${rate}%`} detail={`${stats.valid_frames} of ${stats.recovery_rate.expected_total_frames ?? stats.total_frames_scanned} frames`} tone="blue" /><Stat icon={<FileVideo />} label="Frames recovered" value={stats.valid_frames.toLocaleString()} detail={`${stats.sequences_found} sequence${stats.sequences_found === 1 ? '' : 's'} reconstructed`} tone="green" /><Stat icon={<AlertTriangle />} label="Rejected frames" value={stats.invalid_frames.toLocaleString()} detail={Object.keys(stats.invalid_by_rejection_reason).join(', ') || 'None'} tone="red" /><Stat icon={<Clock3 />} label="Gaps detected" value={stats.gaps_detected.toLocaleString()} detail="Temporal discontinuities" tone="amber" /></div>
...
function ClipCard({ file }: { file: RecoveredFile }) { const jobId = file.path.match(/[\\/]([a-f0-9]{32})[\\/]/)?.[1] ?? ''; return <article className="clip-card"><div className="video-frame">{file.filename.endsWith('.mp4') ? <video controls preload="metadata" src={fileUrl(jobId, file.filename)} /> : <div className="raw-preview"><FileVideo size={28} /><span>Raw stream artifact</span></div>}<span className="clip-status" /></div><div className="clip-info"><strong>{file.filename}</strong><span>{new Date(file.start_ts).toLocaleString()} · {file.frame_count} frames</span></div></article> }
