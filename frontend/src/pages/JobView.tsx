...
export function JobView({ jobId, onReport }: JobViewProps) {
  ...
  const [tab, setTab] = useState<'overview' | 'files' | 'timeline'>('files')
...
function DoneContent({ report, files, view, setView, filter, setFilter, searchQuery, setSearchQuery, copied, setCopied }: { report: Report; files: Report['recovered_files']; view: 'table' | 'grid'; setView: (view: 'table' | 'grid') => void; filter: string; setFilter: (value: string) => void; searchQuery: string; setSearchQuery: (value: string) => void; copied: string; setCopied: (value: string) => void }) {
  ...
  return <>
    <div className="stat-grid"><Stat icon={<ShieldCheck />} label="Recovery rate" value={`${rate}%`} detail={`${stats.valid_frames} of ${stats.recovery_rate.expected_total_frames ?? stats.total_frames_scanned} frames · ${stats.recovery_rate.recovery_rate_basis}`} tone="blue" /><Stat icon={<FileVideo />} label="Frames recovered" value={stats.valid_frames.toLocaleString()} detail={`${stats.sequences_found} sequence${stats.sequences_found === 1 ? '' : 's'} reconstructed`} tone="green" /><Stat icon={<AlertTriangle />} label="Rejected frames" value={stats.invalid_frames.toLocaleString()} detail={topRejection ? `${topRejection[0]} (${topRejection[1]})` : 'None'} tone="red" /><Stat icon={<Clock3 />} label="Gaps detected" value={stats.gaps_detected.toLocaleString()} detail="Temporal discontinuities" tone="amber" /></div><section className="recovery-findings"><div><span className="section-kicker">Recovery findings</span><h2>What the image yielded</h2><p>DeepTrace found {stats.total_frames_scanned} frame candidates in the source image, recovered {stats.valid_frames} valid frames, and retained {stats.invalid_frames} rejected candidates for audit. Rejected candidates represent corruption or incomplete records found while scanning fragmented/deleted image regions.</p></div><div className="findings-meta"><span><b>Validation</b>{report.validation_level || stats.validation_level || 'full'}</span><span><b>Sequences</b>{stats.sequences_found}</span><span><b>Gaps</b>{stats.gaps_detected}</span></div>{report.parsing_notes && <p className="parsing-note">{report.parsing_notes}</p>}</section><div className="rejection-breakdown">{Object.entries(rejectionReasons).map(([reason, count]) => <span key={reason}><b>{reason}</b> {count}</span>)}</div>
...
function FileTable({ files, copied, onCopy }: { files: Report['recovered_files']; copied: string; onCopy: (value: string) => void }) { return <div className="table-wrap"><table><thead><tr><th>Preview</th><th>Artifact</th><th>Timestamp</th><th>Frames</th><th>Size</th><th>SHA-256</th><th /></tr></thead><tbody>{files.map((file) => <tr key={file.filename}><td><InlineClipPreview file={file} /></td><td><div className="file-name"><FileVideo size={17} /><span>{file.filename}<small>{file.is_demo ? 'Playable MP4' : 'Raw recovery fallback'}</small></span></div></td><td className="mono">{new Date(file.start_ts).toLocaleString()}</td><td>{file.frame_count}</td><td>{(file.size_bytes / 1024).toFixed(1)} KB</td><td><button className="hash-button mono" onClick={() => onCopy(file.sha256)} title="Copy SHA-256">{copied === file.sha256 ? 'Copied' : `${file.sha256.slice(0, 12)}…`} <Copy size={13} /></button></td><td><a className="icon-button" href={fileUrl((file.path.match(/[\\/]([a-f0-9]{32})[\\/]/)?.[1] ?? ''), file.filename)} download title="Download"><Download size={16} /></a></td></tr>)}</tbody></table></div> }
function InlineClipPreview({ file }: { file: RecoveredFile }) { const jobId = file.path.match(/[\\/]([a-f0-9]{32})[\\/]/)?.[1] ?? ''; const [open, setOpen] = useState(false); if (!file.filename.toLowerCase().endsWith('.mp4')) return <div className="table-raw-preview"><FileVideo size={18} /></div>; return open ? <video className="table-player" controls autoPlay preload="metadata" src={fileUrl(jobId, file.filename)} /> : <button className="video-thumb table-thumb" onClick={() => setOpen(true)} title="Open video player"><video muted preload="metadata" src={fileUrl(jobId, file.filename)} /><span className="play-mark">▶</span></button> }
function ClipCard({ file }: { file: RecoveredFile }) { const jobId = file.path.match(/[\\/]([a-f0-9]{32})[\\/]/)?.[1] ?? ''; const [open, setOpen] = useState(false); const [events, setEvents] = useState<MotionEvent[] | null>(null); const [motionSummary, setMotionSummary] = useState(''); const [motionError, setMotionError] = useState(''); const runMotion = async () => { try { const result = await analyzeMotion(jobId, file.filename); setEvents(result.motion_events); setMotionSummary(result.summary); setMotionError('') } catch (error) { setMotionError(error instanceof Error ? error.message : 'Motion detection unavailable') } }; return <article className="clip-card"><div className="video-frame">{file.filename.toLowerCase().endsWith('.mp4') ? open ? <video controls autoPlay preload="metadata" src={fileUrl(jobId, file.filename)} /> : <button className="video-thumb" onClick={() => setOpen(true)} title="Open video player"><video muted preload="metadata" src={fileUrl(jobId, file.filename)} /><span className="play-mark">▶</span></button> : <div className="raw-preview"><FileVideo size={28} /><span>Raw stream artifact</span></div>}<span className="clip-status" /></div><div className="clip-info"><strong>{file.filename}</strong><span>{new Date(file.start_ts).toLocaleString()} · {file.frame_count} frames</span>{file.filename.toLowerCase().endsWith('.mp4') && <button className="motion-button" onClick={() => void runMotion()}>Run motion analysis</button>}{events && <section className="motion-events"><b>Motion Detection</b><small>{motionSummary}</small>{events.length ? <ul>{events.map((event) => <li key={`${event.start_sec}-${event.end_sec}`}>{event.start_sec.toFixed(2)}s–{event.end_sec.toFixed(2)}s · intensity {event.intensity.toFixed(1)}</li>)}</ul> : <small>No motion events detected.</small>}</section>}{motionError && <small className="motion-error">{motionError}</small>}</div></article> }
